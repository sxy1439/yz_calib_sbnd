...
void dqdxHists_yzOnly_planes(const char* cintyp, const char* inYZOut) {
  ...
  for(int l=0;l<nplanes;l++){
    ...
    c1->SaveAs(Form("plots_dqdxHists_planes/nosce/finaldqdx_%i_%s.png", l, cintyp));
    c1->SaveAs(Form("plots_dqdxHists_planes/nosce/finaldqdx_%i_%s.pdf", l, cintyp));
  }
  ...
  for(int l=0;l<nplanes;l++){
    ...
    c2->SaveAs(Form("plots_dqdxHists_planes/nosce/dqdx_ratio_%i_%s.png", l, cintyp));
    c2->SaveAs(Form("plots_dqdxHists_planes/nosce/dqdx_ratio_%i_%s.pdf", l, cintyp));
  }
